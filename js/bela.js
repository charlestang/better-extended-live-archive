...
jQuery(
	function($) {
		var BELA = {
			...
			yearClick: function() {
				var that = this;
				$( 'li.year-entry', that.archiveContent ).click(
					function() {
						var entry   = $( this ),
							menu_id = entry.attr( 'menu' ),
							year_id = entry.attr( 'year' );
						that.ajaxRequest(
							{menu: menu_id, year: year_id},
							function() {
								that.eventBinding();
							}
						);
			...
			monthClick: function() {
				var that = this;
				$( 'li.month-entry', that.archiveContent ).click(
					function() {
						var entry    = $( this ),
							menu_id  = entry.attr( 'menu' ),
							year_id  = entry.attr( 'year' ),
						...
						that.ajaxRequest(
							{menu: menu_id, year: year_id, month: month_id},
							function() {
								that.eventBinding();
							}
						);
			...
			categoryClick: function() {
				var that = this;
				$( 'li.category-entry', that.archiveContent ).click(
					function() {
						var entry   = $( this ),
							menu_id = entry.attr( 'menu' ),
							cat_id  = entry.attr( 'cat' );
						that.ajaxRequest(
							{menu: menu_id, cat: cat_id},
							function() {
								that.eventBinding();
							}
						);
			...
			tagClick: function() {
				var that = this;
				$( 'li.tag-entry', that.archiveContent ).click(
					function() {
						var entry   = $( this ),
							menu_id = entry.attr( 'menu' ),
							tag_id  = entry.attr( 'tag' );
						that.ajaxRequest(
							{menu: menu_id, tag: tag_id},
							function() {
								that.eventBinding();
							}
						);
			...
			pageNaviEventHandler: function() {
				...
				BELA.ajaxRequest(
					{menu: menu_id, year: year_id, month: month_id, cat: cat_id, tag: tag_id, page: page_num},
					function() {
						BELA.eventBinding();
					}
				);
