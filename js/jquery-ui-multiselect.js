	...
	$.widget(
		"ui.multiselect",
		{
			...
			_create: function() {
				...
				this.selectedList.height( Math.max( this.element.height() - this.selectedActions.height() + 15,1 ) );
				this.availableList.height( Math.max( this.element.height() - this.availableActions.height() + 15,1 ) );
					...
					this.selectedList.sortable(
						{
							...
							update: function(event, ui) {
								...
								that.selectedList.find( 'li' ).each(
									function() {
							...
							receive: function(event, ui) {
								...
								that.selectedList.children( '.ui-draggable' ).each(
									function() {
				...
				// batch actions
				this.container.find( ".remove-all" ).click(
					function() {
				...
				this.container.find( ".add-all" ).click(
					function() {
						var options = that.element.find( 'option' ).not( ":selected" );
						if (that.availableList.children( 'li:hidden' ).length > 1) {
							that.availableList.children( 'li' ).each(
								function(i) {
									if ($( this ).is( ":visible" )) {
